...
describe('<MaterialLaminate>', () => {
  ...
  beforeEach(() => {
    ...
    wrapper = shallow(<MaterialLaminate material={material} />);
    wrapper.setState({ laminates: material.laminates });
  ...
  it('edit item', () => {
    wrapper.find('#code').value = 1;
    setTimeout(function() {
      wrapper.update();
      expect(wrapper.state().laminates[0].code).toEqual(1);  
      done();
    }, 100);
  });
